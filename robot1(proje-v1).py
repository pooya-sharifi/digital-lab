...
#                 heading_deg= math.degrees(heading)
                phi_des=direction
                Error= phi_des
                zavie_toop=math.degrees(math.atan2(ball_data['direction'][1],ball_data['direction'][0]))
...
#                 if math.fabs(Error) < 0.07:
                a=math.fabs(180-math.fabs(zavie_toop))
                print(f"strength is :{ball_data['strength']}")
                if direction == 0:
...
#                         self.right_motor.setVelocity(0)
#                     if Error<0.5:
                if in_ball_direction !=True  or a<10:
                    print("shooroo be gardesh")
                    sum_Error += Error * 0.1
                    Error_d=( Error - Error_back)/0.1
                    Error_back = Error
...
#                   posht toop residan
                    ...
                    print("&&&&")
                    print(error_distance,sum_error_distance,error_distance_d)
                    kp_1=0.0001
                    ki_1=0.09
                    kd_1=0.09
...
#                     shoot kardan
                    heading_deg= math.degrees(heading)
                    if (error_distance < (260-180) and heading_deg>-170):
#                         hamoon code 3 be samte darvaze
                        print("becharkh samte darvaze")
                        heading_deg= math.degrees(heading)
                        y_co=0
                        x_co=-0.75
...
##                print(vr,vl)
                        self.left_motor.setVelocity(vl)
                        self.right_motor.setVelocity(vr)
#                         hamoon code 3 be shoot,error ham zaviye charkhesh
                        if(Error<40):
                            self.left_motor.setVelocity(8)
                            self.right_motor.setVelocity(8)
#                     error_distance_d<1 dare chape mire vali shak
                    if (direction != 0 or error_distance_d<1):
                        in_ball_direction=False
